...
for ts in u.trajectory:
    loopd1_atoms = u.select_atoms('segid F and resid 224-227')
    loopd2_atoms = u.select_atoms('segid F and resid 204-208')
    dna_atoms = u.select_atoms('segid G and name S')
    atp_atoms = u.select_atoms('segid M')

    loopd1_atoms_com = loopd1_atoms.center_of_mass()
    loopd2_atoms_com = loopd2_atoms.center_of_mass()
    
    minimum_value = 99
    index = 99
    
    for i, each_dna in enumerate(dna_atoms):
         dist = np.linalg.norm(loopd2_atoms_com - each_dna.position)
         if dist < minimum_value:
             minimum_value = dist
             index = i+1
         #print(each_dna.position)

    print(index)
    #xyz_1 = np.array(loopd1_atoms_com)
    #xyz_2 = np.array(loopd2_atoms_com)
    #xyz_3 = np.array(dna_atoms_com)
