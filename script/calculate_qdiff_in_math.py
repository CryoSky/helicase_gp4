...
# Written by Shikai Jin on 2021-Feb-10, latest modified on 2021-Feb-10
# A script of calculating Qdiff value
# Example in Linux: python this.py output.pdb model_001.pdb model_050.pdb 0.2747 qdiff.txt
                ...
                if chain_j.full_id[-1] == chain_b_id:
                    ...
                    for i, residue_i in enumerate(chain_i.get_residues()):
                        ...
                        for j, residue_j in enumerate(chain_j.get_residues()):
                            ...
                            sigma_ij = ((abs(i - j) + 1) ** 0.15)
                            #sigma_ij = abs(i - j) ** 0.15
                            gamma_ij = 1.0
...
def compute_qinterface_for_traj(traj_ca_atoms_1, traj_ca_atoms_2, reference_qinterface_data_1, reference_qinterface_data_2):
    counts = 0
    q_sum = 0
    length = len(traj_ca_atoms_1)
    for each_line_in_chainA in range(length):
        for each_line_in_chainB in range(length):
            #print("first is:" + str(traj_ca_atoms_1[each_line][0]-1))
            #print("second is:" + str(traj_ca_atoms_2[each_line[1]-1]))
            #print("The index A is:" + str(each_line_in_chainA))
            #print("The index B is:" + str(each_line_in_chainB))
            #print("The count is:" + str(counts))
            rij_traj = vabs(vector(traj_ca_atoms_1[each_line_in_chainA], traj_ca_atoms_2[each_line_in_chainB]))
            this_q = math.exp(-(rij_traj - reference_qinterface_data_1[counts][3]) ** 2 / (2 * (reference_qinterface_data_1[counts][4] ** 2))) - math.exp(-(rij_traj - reference_qinterface_data_2[counts][3]) ** 2 / (2 * (reference_qinterface_data_2[counts][4] ** 2)))
            q_sum += this_q
            counts += 1
    #print(counts)
    #print(q_sum/counts)
...
def compute_traj(reference_file_1, reference_file_2, trajectory_file, scaling, output):
    reference_qinterface_data_1 = compute_qinterface_for_reference(reference_file_1)
    reference_qinterface_data_2 = compute_qinterface_for_reference(reference_file_2)
    ...
    for i in range(int(n_snapshot)):
        ...
        q = compute_qinterface_for_traj(traj_ca_atoms_1, traj_ca_atoms_2, reference_qinterface_data_1, reference_qinterface_data_2)
        qdiff = (q+scaling)/(scaling*2)
        print(qdiff)
        out.write(str(round(qdiff, 3)) + "\n")
    out.close()
...
def main():
    ...
    parser.add_argument("topology_1", help="The file name of topology 1 for calculation, qdiff value for this structure is positive", type=str)
    parser.add_argument("topology_2", help="The file name of topology 2 for calculation, qdiff value for this structure is negative", type=str)
    parser.add_argument("scaling", help="The file Qdiff value between topology 1 and 2 in positive, for gp4 is 0.827", default=0.827, type=float)
    parser.add_argument("output", help="The file name of output",
                        type=str)
    args = parser.parse_args()
    trajectory = args.trajectory
    topology_1 = args.topology_1
    topology_2 = args.topology_2
    scaling = args.scaling
    output = args.output

    compute_traj(topology_1, topology_2, trajectory, scaling, output)
