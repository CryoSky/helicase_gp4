...
def compute_qinterface_for_traj(traj_ca_atoms_1, traj_ca_atoms_2, reference_qinterface_data_1, reference_qinterface_data_2):
    counts = 0
    q_sum = 0
    length = len(reference_qinterface_data_1)
    for each_line in range(len(reference_qinterface_data_1)):
        counts += 1
            #print("first is:" + str(traj_ca_atoms_1[each_line[0]-1]))
            #print("second is:" + str(traj_ca_atoms_2[each_line[1]-1]))
        rij_traj = vabs(vector(traj_ca_atoms_1[reference_qinterface_data_1[each_line][0]-1], traj_ca_atoms_2[reference_qinterface_data_1[each_line][1]-1]))
        q_sum += math.exp(-(rij_traj - reference_qinterface_data_1[each_line][3]) ** 2 / (2 * (reference_qinterface_data_1[each_line][4] ** 2)))
        q_sum -= math.exp(-(rij_traj - reference_qinterface_data_2[each_line][3]) ** 2 / (2 * (reference_qinterface_data_1[each_line][4] ** 2)))
    
    #print(q_sum/counts)
    return q_sum/counts


def compute_traj(reference_file_1, reference_file_2, trajectory_file, output):
...
def main():
    ...
    parser.add_argument("topology_1", help="The file name of topology for align, any frame of trajectory is fine", type=str)
    parser.add_argument("topology_2", help="The file name of topology for align, any frame of trajectory is fine", type=str)
    ...
    compute_traj(topology_1, topology_2, trajectory, output)
