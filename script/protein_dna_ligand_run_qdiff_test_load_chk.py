...
for protein_force_name in openAWSEMforces:
    ...
    if protein_force_name in ['contact']:
        ...
        protein_force_current = openAWSEMforces[protein_force_name](protein, q_bin_min, reference_pdb_file_1="../model_001_protein_cg_correct.pdb", reference_pdb_file_2="../model_050_protein_cg_correct.pdb", k_qbias=5000)
    else:
        protein_force_current = openAWSEMforces[protein_force_name](protein)
...
simulation.loadCheckpoint(f'checkpoint.chk')

#simulation.context.setPositions(coord)
#simulation.context.setVelocitiesToTemperature(300*simtk.openmm.unit.kelvin, RANDOM_SEED)
...
dcd_reporter=simtk.openmm.app.DCDReporter(f'output.dcd', 1)
pdb_reporter=simtk.openmm.app.PDBReporter(f'output.pdb', 1)
energy_reporter=simtk.openmm.app.StateDataReporter(sys.stdout, 1, step=True,time=True,
                                                   potentialEnergy=True, temperature=True)
simulation.reporters.append(dcd_reporter)
simulation.reporters.append(pdb_reporter)
simulation.reporters.append(energy_reporter)
#simulation.step(1000)
#simulation.saveCheckpoint(f'checkpoint.chk')
#Run simulation
